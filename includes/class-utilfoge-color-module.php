...
class UTILFOGE_Color_Module {
	...
	public function init() {
		...
		// AJAX endpoints
		add_action( 'wp_ajax_utilfoge_parse_colors', array( $this, 'ajax_parse_colors' ) );
		add_action( 'wp_ajax_utilfoge_expand_colors', array( $this, 'ajax_expand_colors' ) );
		add_action( 'wp_ajax_utilfoge_save_expanded', array( $this, 'ajax_save_expanded' ) );
		add_action( 'wp_ajax_utilfoge_sync_dark_auto', array( $this, 'ajax_sync_all_dark' ) );

		// CSS injection
		add_action( 'wp_enqueue_scripts', array( $this, 'enqueue_frontend_styles' ), 15 );

		// ── Core filter: merge UTILFOGE colors into GP Global Colors on every read ──
	...
	// ─────────────────────────────────────────
	// CORE FILTER: Merge UTILFOGE → GP Global Colors
	// ─────────────────────────────────────────
	...
	public function register_customizer( $wp_customize ) {
		...
		// Custom control
		require_once UTILFOGE_PLUGIN_DIR . 'includes/class-utilfoge-color-import-control.php';
		$wp_customize->add_control(
			new UTILFOGE_Color_Import_Control(
				$wp_customize,
				'utilfoge_expanded_colors',
				array(
					'label'   => 'Color Import & Expansion',
					'section' => 'utilfoge_color_management',
				)
			)
		);
	}
	...
	public function enqueue_customizer_assets() {
		wp_enqueue_style(
			...
			UTILFOGE_PLUGIN_URL . 'assets/css/utilfoge-color-module.css',
			array(),
			UTILFOGE_VERSION
		);
		...
		wp_enqueue_script(
			'utilfoge-color-module',
			UTILFOGE_PLUGIN_URL . 'assets/js/utilfoge-color-module.js',
	...
	public function ajax_parse_colors() {
		check_ajax_referer( 'utilfoge_color_module', 'nonce' );
		...
		$parsed = UTILFOGE_Color_Math::parse_import( $raw );
	...
	public function ajax_expand_colors() {
		...
		foreach ( $colors as $item ) {
			...
			$expanded = UTILFOGE_Color_Math::expand_color( $slug, $hex, $options );
			...
			$result[] = array(
				...
				'dark_counterparts' => UTILFOGE_Color_Math::extract_dark_counterparts( $expanded ),
			);
		}
	...
	public function ajax_save_expanded() {
		...
		update_option( 'utilfoge_expanded_colors', $sanitized );
	...
	private function sync_dark_counterparts( $expanded ) {
		...
		set_theme_mod( 'utilfoge_dark_global_colors', $final_dark_colors );
