...
class WPIDS_Settings {
	...
	public function render_settings_page() {
		...
		<div class="gpu-dashboard-header">
			<div class="gpu-dashboard-header__title">
				<h1>
					...
					<?php esc_html_e( 'GeneratePress Utility', 'generatepress-utility' ); ?>
				</h1>
			</div>
			<div class="gpu-dashboard-header__nav">
				<a href="#" class="gpu-active"><?php esc_html_e( 'Utilities', 'generatepress-utility' ); ?></a>
			...
			<div class="gpu-dashboard-content">
				...
				<div class="gpu-module-list">
					...
					foreach ( $modules as $id => $module ) : 
						...
						// Mimic GP Premium dynamic shadow
					...
					<div class="gpu-module-row <?php echo $is_active ? 'gpu-active' : 'gpu-inactive'; ?>" style="<?php echo esc_attr( $row_style ); ?>">
							...
							<h3>
								...
								<?php if ( $is_active && ! empty( $module['is_page'] ) && ! empty( $module['action_link'] ) ) : ?>
									<a class="gpu-module-action" href="<?php echo esc_url( $module['action_link'] ); ?>">
										<?php esc_html_e( 'Open Tool  →', 'generatepress-utility' ); ?>
									</a>
								<?php endif; ?>
							</h3>
						...
						<div class="gpu-module-actions">
							...
							if ( $show_message ) :
								$msg_text = ( $_GET['message'] === 'activated' ) ? __( 'Module activated.', 'generatepress-utility' ) : __( 'Module deactivated.', 'generatepress-utility' );
							?>
							...
							<?php if ( $is_active && ! empty( $module['action_link'] ) && empty( $module['is_page'] ) ) : ?>
								<div class="gpu-module-settings">
									<a href="<?php echo esc_url( $module['action_link'] ); ?>" class="components-button is-tertiary has-icon" title="<?php esc_attr_e( 'Settings', 'generatepress-utility' ); ?>">
							...
							<?php if ( $is_active ) : ?>
								<a href="<?php echo esc_url( $toggle_url ); ?>" class="components-button is-secondary"><?php esc_html_e( 'Deactivate', 'generatepress-utility' ); ?></a>
							<?php else : ?>
								<a href="<?php echo esc_url( $toggle_url ); ?>" class="components-button is-primary"><?php esc_html_e( 'Activate', 'generatepress-utility' ); ?></a>
							<?php endif; ?>
						</div>
					</div>
					<?php endforeach; ?>
				...
				?>
				<div class="gpu-section-title">
					<h2><?php esc_html_e( 'Synced CSS', 'generatepress-utility' ); ?></h2>
				</div>
				<div class="gpu-advanced-section">
					<h3><?php esc_html_e( 'Advanced CSS Editor', 'generatepress-utility' ); ?></h3>
					<p><?php esc_html_e( 'Write any advanced CSS here to adjust your style both in Editor and . Feel Free to use any @property or HTML syntax.', 'generatepress-utility' ); ?></p>
					<form method="post" action="options.php">
						<?php settings_fields( 'wpids_utility_settings_group' ); ?>
						<textarea name="wpids_utility_raw_css" rows="10" placeholder="/* Write your CSS here... */"><?php echo esc_textarea( get_option( 'wpids_utility_raw_css', '' ) ); ?></textarea>
						<div class="gpu-form-footer">
							<input type="submit" name="submit" id="wpids-save-css-btn" class="button button-primary" value="<?php esc_attr_e( 'Save Settings', 'generatepress-utility' ); ?>">
						</div>
					</form>
				</div>
				<?php endif; ?>
		...
		(function() {
			...
			var form = document.querySelector('.gpu-advanced-section form');
			if ( ! form ) return;
			form.addEventListener( 'submit', function() {
				...
				setTimeout( function() {
					...
					setTimeout( function() { btn.value = originalValue; }, 3000 );
				}, 100 );
			});
		})();
		</script>
